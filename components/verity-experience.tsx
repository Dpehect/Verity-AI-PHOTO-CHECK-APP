...
const timeline = [
  { id: "01", label: "Captured", meta: "Leica SL3 · 06:42 UTC", detail: "Original capture signed at source", tone: "valid" },
  { id: "02", label: "Adjusted", meta: "Lightroom · 07:18 UTC", detail: "Exposure and color profile changed", tone: "edit" },
  { id: "03", label: "Published", meta: "Northstar News · 08:03 UTC", detail: "Publisher identity cryptographically signed", tone: "valid" },
  { id: "04", label: "Verified", meta: "Verity · just now", detail: "Credential chain intact", tone: "valid" },
];

const samples = [
  { name: "Editorial capture", type: "JPG · 18.4 MB", status: "Verified", confidence: 100, color: "sample-blue" },
    ...
    () => {
      ...
      media.add(
        ...
        (context) => {
          ...
          gsap.to("[data-orbit]", {
            rotate: 18,
            scrollTrigger: { trigger: "[data-hero]", start: "top top", end: "bottom top", scrub: 1.2 },
          });
          ...
          gsap.utils.toArray<HTMLElement>("[data-trace-row]").forEach((row, index) => {
            gsap.from(row, {
              opacity: 0.18,
              x: index % 2 === 0 ? -24 : 24,
  ...
  return (
    <main ref={root}>
      ...
      <header className="nav shell">
        ...
        {menuOpen && <div className="mobile-menu"><a href="#how" onClick={() => setMenuOpen(false)}>How it works</a><a href="#verify" onClick={() => setMenuOpen(false)}>Verify</a><a href="#workspace" onClick={() => setMenuOpen(false)}>For teams</a></div>}
      </header>

      <section id="top" className="hero" data-hero ref={hero}>
        <div className="hero__noise" />
        <div className="hero__eyebrow shell"><span><i /> Digital evidence infrastructure</span><span>Scroll to inspect <ArrowDown size={13} /></span></div>
        <div className="hero__copy shell">
          <h1 aria-label="See the story behind every file">
            <span className="line"><span data-hero-line>See the story</span></span>
            <span className="line line--offset"><span data-hero-line>behind every file.</span></span>
          </h1>
          <p>Inspect content credentials, trace edit history, and understand where digital media comes from.</p>
        </div>
        ...
        <div className="hero__footer shell"><p>Built on the open C2PA standard</p><div><span>TRACE</span><span>COMPARE</span><span>VERIFY</span></div></div>
      </section>

      <section id="how" className="manifest section shell">
        <div className="section-heading" data-reveal>
          <p className="kicker">01 / Provenance, made visible</p>
          <h2>A file is not a moment.<br/><em>It is a chain of events.</em></h2>
          <p className="section-intro">Verity reads the cryptographically signed history attached to digital media and translates it into evidence people can understand.</p>
        </div>

        <div className="trace">
          <div className="trace__visual" data-reveal>
            <div className="trace__frame"><div className="trace__scene"><div className="trace__sun"/><div className="trace__hill trace__hill--one"/><div className="trace__hill trace__hill--two"/></div><span>ACTIVE MANIFEST</span></div>
            <div className="trace__stamp"><Fingerprint size={23}/><span>SHA-256</span><b>8f42...a91c</b></div>
          </div>
          <div className="trace__list">
            {timeline.map((item) => (
              <article key={item.id} data-trace-row>
                <div className={`trace__dot trace__dot--${item.tone}`}><span>{item.id}</span></div>
                <div><h3>{item.label}</h3><p>{item.meta}</p><small>{item.detail}</small></div>
                <Check size={17}/>
              </article>
            ))}
          </div>
      ...
      <section id="verify" className="verify section">
        <div className="shell">
          <div className="section-heading section-heading--light" data-reveal>
            ...
            <h2>Don&apos;t take our word for it.<br/><em>Look closer.</em></h2>
          </div>

          <div className="verify-console" data-reveal>
            <div className="verify-console__sidebar">
              <div className="console-title"><FileSearch size={17}/><span>SAMPLE EVIDENCE</span></div>
              {samples.map((item, index) => (
                <button key={item.name} className={index === activeSample ? "is-active" : ""} onClick={() => setActiveSample(index)}>
                  <span className={`sample-thumb ${item.color}`}><ScanLine size={18}/></span>
            ...
            <aside className="verify-console__result">
              ...
              <div className={`result-seal result-seal--${activeSample}`}><ShieldCheck size={30}/><span>{sample.confidence || "—"}<small>{sample.confidence ? "%" : ""}</small></span></div>
              <h3>{sample.status}</h3>
              <p>{activeSample === 0 ? "The credential is valid and its complete history is available." : activeSample === 1 ? "The credential is valid. Declared edits were found in its history." : "No Content Credential was found. This does not mean the asset is false."}</p>
              <dl>
                <div><dt>Signer</dt><dd>{activeSample === 2 ? "Unknown" : "Northstar News"}</dd></div>
                <div><dt>Signature</dt><dd>{activeSample === 2 ? "Not found" : "Valid"}</dd></div>
        ...
        <div className="workspace__ui" data-reveal>
          ...
          <div className="workspace__body">
            ...
            <div className="workspace__table">
              ...
              {["Northern light.jpg", "City archive-04.tif", "Interview-final.mp4", "Campaign-export.png"].map((name, i) => <div className="table-row" key={name}><span><i className={`mini-media mini-media--${i}`}/><b>{name}</b></span><span><i className={i === 3 ? "warn" : "ok"}/>{i === 3 ? "Review" : "Verified"}</span><span>{i % 2 ? "Archive team" : "Northstar News"}</span><span>{i + 2} min ago</span></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="closing">
        ...
        <div className="closing__seal" data-reveal><ShieldCheck/><span>PROOF<br/>FOUND</span></div>
